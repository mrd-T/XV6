...
void find(char *path, char *str) { // 类Unix系统中，目录被视为一种特殊类型的文件
  ...
  if (fstat(fd, &st) <
      0) { // 通过文件描述符将对应的文件信息放入文件结构体stat中,若失败则返回-1
  ...
  case T_DIR: // 判定为目录
    ...
    *p = '/';
    ...
    while (read(fd, &de, sizeof de) ==
           sizeof de) { // 使用read从目录文件中读取目录条目，处理目录中文件
      if (de.inum == 0) // 该目录条目为空或未使用
        continue;
      memmove(p, de.name, DIRSIZ);
      p[DIRSIZ] = 0;
      if (stat(buf, &st) <
          0) { // 通过文件路径将对应的文件信息放入文件结构体stat中，若失败则返回-1
      ...
      if (st.type == T_DEVICE || st.type == T_FILE) { // 判断为非目录文件
        if (!strcmp(str, get_name(buf)))
          ...
          st.type == T_DIR && strcmp(".", get_name(buf)) &&
          strcmp(
              "..",
              get_name(
                  buf))) // 判定为子目录，递归处理，注意不要重复进入本目录以及父目录
        find(buf, str);
    }
    break;
  }
